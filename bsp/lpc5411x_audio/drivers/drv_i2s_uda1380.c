...
void UDA1380_REG_Write(uint8_t reg, uint16_t val)
{
	...
	i2c_xfer.slaveAddr = I2CDEV_UDA1380_ADDR;
	tx_buff[0] = reg;
	...
	if(i2c_xfer.status == I2CM_STATUS_OK) {
		rt_kprintf("UDA1380_REG_Write OK\r\n");
	} else {
		rt_kprintf("UDA1380_REG_Write FA\r\n");
	}
}
...
uint16_t UDA1380_REG_Read(uint8_t reg) {
	...
	i2c_io(&i2c_xfer);
	if (i2c_xfer.status == I2CM_STATUS_OK) {
		rt_kprintf("UDA1380_REG_Read OK\r\n");
		return (rx_buff[0] << 8) | rx_buff[1];
	} else {
		rt_kprintf("UDA1380_REG_Read FA\r\n");
		return 0;
	}
}
