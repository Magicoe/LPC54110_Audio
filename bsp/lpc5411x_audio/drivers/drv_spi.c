...
void DMA_IRQHandler(void)
{
	...
	if ((Chip_DMA_GetIntStatus(LPC_DMA) & DMA_INTSTAT_ACTIVEERRINT) != 0) {
		/* This shouldn't happen for this simple DMA example, so set the LED
		   ...
		   an abort. */
		Chip_DMA_DisableChannel(LPC_DMA, DMA_TX_CHANNEL);
		while ((Chip_DMA_GetBusyChannels(LPC_DMA) & (1 << DMA_TX_CHANNEL)) != 0) {}
		Chip_DMA_AbortChannel(LPC_DMA, DMA_TX_CHANNEL);
		Chip_DMA_ClearErrorIntChannel(LPC_DMA, DMA_TX_CHANNEL);
		Chip_DMA_EnableChannel(LPC_DMA, DMA_TX_CHANNEL);
			
		Chip_DMA_DisableChannel(LPC_DMA, DMA_RX_CHANNEL);
		while ((Chip_DMA_GetBusyChannels(LPC_DMA) & (1 << DMA_RX_CHANNEL)) != 0) {}
		Chip_DMA_AbortChannel(LPC_DMA, DMA_RX_CHANNEL);
		Chip_DMA_ClearErrorIntChannel(LPC_DMA, DMA_RX_CHANNEL);
		Chip_DMA_EnableChannel(LPC_DMA, DMA_RX_CHANNEL);
	}

    /* Clear DMA interrupt for the channel */
	if ( (Chip_DMA_GetIntStatus(LPC_DMA) & DMA_INTSTAT_ACTIVEINT) != 0 ) {
		if ( Chip_DMA_GetActiveIntAChannels(LPC_DMA) & (1 << DMA_RX_CHANNEL) ) {
			Chip_DMA_ClearActiveIntAChannel(LPC_DMA, DMA_RX_CHANNEL);
			spi1_rx_flag = 1;
			if( spi1_tx_flag == 1){
		...
		if ( Chip_DMA_GetActiveIntAChannels(LPC_DMA) & (1 << DMA_TX_CHANNEL) ) {
			Chip_DMA_ClearActiveIntAChannel(LPC_DMA, DMA_TX_CHANNEL);
			spi1_tx_flag = 1;
			if( spi1_rx_flag == 1){
				spi1_tx_flag = spi1_rx_flag = 0;
				rt_sem_release(&spi1_tx_done_sem);		
...
static void DMA_Init(void)
{
	...
	LPC_DMA->DMACOMMON[0].ENABLESET |= (1UL<<DMA_TX_CHANNEL) | (1UL<<DMA_RX_CHANNEL);
	LPC_DMA->DMACOMMON[0].INTENSET  |= (1UL<<DMA_TX_CHANNEL) | (1UL<<DMA_RX_CHANNEL);
	LPC_DMA->DMACH[DMA_TX_CHANNEL].CFG = DMA_CFG_PERIPHREQEN | DMA_CFG_TRIGBURST_SNGL | DMA_CFG_CHPRIORITY(1);
	LPC_DMA->DMACH[DMA_RX_CHANNEL].CFG = DMA_CFG_PERIPHREQEN | DMA_CFG_TRIGBURST_SNGL | DMA_CFG_CHPRIORITY(0);
...
static void DMA_Configuration(uint8_t * send_addr, uint8_t * recv_addr, rt_size_t size)
{
    ...
    */
    Chip_SPI_ClearStatus(SDCARD_SPI, SPI_STAT_RXOV | SPI_STAT_TXUR | SPI_STAT_SSA | SPI_STAT_SSD);
	// DMA Channle 6 RX
	if(recv_addr != NULL) {
		spi1_rx_dmaDesc.source    = DMA_ADDR(&SDCARD_SPI->FIFORD);
	...
	if(send_addr != NULL) {
		...
		spi1_tx_dmaDesc.dest      = DMA_ADDR(&SDCARD_SPI->FIFOWR);
    ...
    pDesc[DMA_RX_CHANNEL] = spi1_rx_dmaDesc;
	pDesc[DMA_TX_CHANNEL] = spi1_tx_dmaDesc;
	
	// wait until DMA channel is not active
	for (retry = 5432; retry !=0; retry--) {
		if (0 == (LPC_DMA->DMACOMMON[0].ACTIVE & (1UL<<DMA_TX_CHANNEL | 1UL<<DMA_RX_CHANNEL)))
			break;
	}	
	
	if(recv_addr != NULL) {
		LPC_DMA->DMACH[DMA_RX_CHANNEL].XFERCFG = (DMA_XFERCFG_CFGVALID |  DMA_XFERCFG_SETINTA | DMA_XFERCFG_SWTRIG   |
	...
	if(send_addr != NULL) {
		LPC_DMA->DMACH[DMA_TX_CHANNEL].XFERCFG = (DMA_XFERCFG_CFGVALID |  DMA_XFERCFG_SETINTA | DMA_XFERCFG_SWTRIG   |
...
static rt_err_t configure(struct rt_spi_device* device, struct rt_spi_configuration* configuration)
{
	...
	if(spi1_config_flag == 0)
    {
        ...
        if(configuration->data_width <= 8) {
            Chip_SPI_SetXferSize(SDCARD_SPI, 8);
        }
        else if(configuration->data_width <= 16) {
            Chip_SPI_SetXferSize(SDCARD_SPI, 16);
        }
        else {
            return RT_EIO;
        }
        ...
        Chip_SPI_ConfigureSPI(SDCARD_SPI, &spiSetup);
        Chip_SPI_SetCSPolLow(SDCARD_SPI, 0);
        ...
        Chip_SPIM_DelayConfig(SDCARD_SPI, &masterDelay);
        //SDCARD_SPI->FIFOTRIG = (0<<16)|(1 << 1);
        Chip_SPI_SetFIFOTrigLevel(SDCARD_SPI, 0, 0);
		
		
//		uart->UART->FIFOINTENSET |= (1<<3);
#ifdef SPI_USE_DMA
        SDCARD_SPI->FIFOCFG = ((1 << 4) | SPI_FIFOCFG_ENABLETX | SPI_FIFOCFG_ENABLERX | SPI_FIFOCFG_DMATX | SPI_FIFOCFG_DMARX | SPI_FIFOCFG_WAKETX | SPI_FIFOCFG_WAKERX );
        Chip_SPI_DisableInts(SDCARD_SPI, SPI_INT_TXDYEN | SPI_INT_RXDYEN);
        DMA_Init();
#endif
	}
    /* baudrate */
    ret = Chip_SPIM_SetClockRate(SDCARD_SPI, configuration->max_hz);
...
//		Chip_SPI_EnableInts(SDCARD_SPI, (SPI_INT_SSAEN | SPI_INT_SSDEN));
    return RT_EOK;
};
...
//        rt_kprintf("rt_sem_take OK %x\r\n", err);
...
#endif
    {
        if(config->data_width <= 8)
        {
            SDCARD_SPI->FIFOCFG = ((1 << 4) | SPI_FIFOCFG_ENABLETX | SPI_FIFOCFG_ENABLERX );
...
//		Chip_GPIO_Init(LPC_GPIO);
    ...
    if(SPI == SDCARD_SPI)
    {
    	lpc_spi->SPI = SDCARD_SPI;
#ifdef SPI_USE_DMA

#endif
        /* Initialize SPI controller */
        rt_kprintf("Init SPI %s %s\r\n", __FILE__, __LINE__);
        ret = Chip_SPI_Init(SDCARD_SPI);
        LPC_ASSERT(!ret, __FILE__, __LINE__);
        Chip_SPI_Enable(SDCARD_SPI);
    }
    else
    {
        return RT_ENOSYS;
...
void rt_hw_spi_init(void)
{
	...
	lpc_spi_register(SDCARD_SPI, &lpc_spi, "spi1");
