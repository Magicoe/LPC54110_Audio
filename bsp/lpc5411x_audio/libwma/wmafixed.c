 ...
 */
volatile int32_t fsc_x, fsc_x1, fsc_y, fsc_y1; 
volatile unsigned long fsc_z, fsc_z1;
...
//    unsigned long z, z1;
    ...
    fsc_x = cordic_circular_gain;
    fsc_y = 0;
    fsc_z = phase;

    /* The phase has to be somewhere between 0..pi for this to work right */
    if (fsc_z < 0xffffffff / 4) {
        /* z in first quadrant, z += pi/2 to correct */
        fsc_x = -fsc_x;
        fsc_z += 0xffffffff / 4;
    } else if (fsc_z < 3 * (0xffffffff / 4)) {
        /* z in third quadrant, z -= pi/2 to correct */
        fsc_z -= 0xffffffff / 4;
    } else {
        /* z in fourth quadrant, z -= 3pi/2 to correct */
        fsc_x = -fsc_x;
        fsc_z -= 3 * (0xffffffff / 4);
    }
