...
// Magicoe fixed32 coefsarray[MAX_CHANNELS][BLOCK_MAX_SIZE] ;
fixed32 *coefsarray[MAX_CHANNELS];
/*decode and window into IRAM on targets with at least 80KB of codec IRAM*/
// Magicoe fixed32 frame_out_buf[MAX_CHANNELS][BLOCK_MAX_SIZE * 2] ;
fixed32 *frame_out_buf[MAX_CHANNELS];
...
// Magicoe volatile fixed32 *stat0;
//volatile fixed32 *stat1;
//volatile fixed32 *stat2;
//volatile fixed32 *stat3;
//volatile fixed32 *stat4;

uint16_t *runtabarray[2], *levtabarray[2];                                        //these are VLC lookup tables

// Magicoe uint16_t runtab0[1336], runtab1[1336], levtab0[1336], levtab1[1336];			//these could be made smaller since only one can be 1336
volatile uint16_t *runtab0;
volatile uint16_t *runtab1;
volatile uint16_t *levtab0;
volatile uint16_t *levtab1; 
...
VLC_TYPE (*vlcbuf1)[2];//[VLCBUF1SIZE][2];
VLC_TYPE (*vlcbuf2)[2];//[VLCBUF2SIZE][2];
VLC_TYPE (*vlcbuf3)[2];//[VLCBUF3SIZE][2];
VLC_TYPE (*vlcbuf4)[2];//[VLCBUF4SIZE][2];
...
volatile uint32_t wma_dec_i, wma_dec_j, wma_dec_n;
volatile fixed32 wma_dec_j2, wma_dec_j3, wma_dec_alpha;
volatile fixed32 *wma_dec_temp[5];
...
// Magicoe    s->coefs = &coefsarray;
//    s->frame_out = &frame_out_buf;
    (int32_t)s->coefs = *coefsarray[0];
    (int32_t)s->frame_out = *frame_out_buf[0];
    ...
    /* compute the scale factor band sizes for each MDCT block size */
    {
        ...
        for(k = 0; k < s->nb_block_sizes; ++k)
        {
            ...
            tmp1 = high_freq*2;            /* high_freq is a fixed32!*/
            tmp2 = itofix32(s->sample_rate>>1);
    ...
    */
// Magicoe    temp[0] = stat0;
// Magicoe    temp[1] = stat1;
// Magicoe    temp[2] = stat2;
// Magicoe    temp[3] = stat3;
// Magicoe    temp[4] = stat4;
//		stat0 = rt_malloc(2048*4);
//		if(stat0 == NULL) {
//			rt_kprintf("malloc stat0 failed\n");
//			while(1);
//		}
//		rt_kprintf("stat0 malloc size %d addr 0x%x\r\n", 2048*4, stat0);
//		
//		stat1 = rt_malloc(1024*4);
//		if(stat1 == NULL) {
//			rt_kprintf("malloc stat1 failed\n");
//			while(1);
//		}
//		rt_kprintf("stat1 malloc size %d addr 0x%x\r\n", 1024*4, stat1);
//		
//		stat2 = rt_malloc(512*4);
//		if(stat2 == NULL) {
//			rt_kprintf("malloc stat2 failed\n");
//			while(1);
//		}
//		rt_kprintf("stat2 malloc size %d addr 0x%x\r\n", 512*4, stat2);
//		
//		stat3 = rt_malloc(256*4);
//		if(stat3 == NULL) {
//			rt_kprintf("malloc stat3 failed\n");
//			while(1);
//		}
//		rt_kprintf("stat3 malloc size %d addr 0x%x\r\n", 256*4, stat3);
//		
//		stat4 = rt_malloc(128*4);
//		if(stat4 == NULL) {
//			rt_kprintf("malloc stat4 failed\n");
//			while(1);
//		}
//		rt_kprintf("stat4 malloc size %d addr 0x%x\r\n", 128*4, stat4);	
	
    wma_dec_temp[0] = (int32_t *)stat0;
    wma_dec_temp[1] = (int32_t *)stat1;
    wma_dec_temp[2] = (int32_t *)stat2;
    wma_dec_temp[3] = (int32_t *)stat3;
    wma_dec_temp[4] = (int32_t *)stat4;

    /* init MDCT windows : simple sinus window */
    for(wma_dec_i = 0; wma_dec_i < s->nb_block_sizes; wma_dec_i++)
    {
//			rt_kprintf("nb_block_sizes 0x%x\r\n", s->nb_block_sizes);
			wma_dec_j = 0;
			wma_dec_j2 = 0;
			wma_dec_n = 1 << (s->frame_len_bits - wma_dec_i);
			//window = av_malloc(sizeof(fixed32) * n);
			window = wma_dec_temp[wma_dec_i];
			...
			wma_dec_alpha = (1<<15)>>(s->frame_len_bits - wma_dec_i + 1);   /* this calculates 0.5/(2*n) */
//			rt_kprintf("wma_dec_i 0x%x, wma_dec_j = 0x%x, wma_dec_n=0x%x wma_dec_alpha 0x%x, frame_len_bits 0x%x\r\n", wma_dec_i, wma_dec_j, wma_dec_n, wma_dec_alpha, s->frame_len_bits);
			for(wma_dec_j=0;wma_dec_j<wma_dec_n;wma_dec_j++) {
				wma_dec_j2 = itofix32(wma_dec_j) + 0x8000;
// Magicoe window[wma_dec_j] = fsincos(fixmul32(wma_dec_j2, wma_dec_alpha)<<16, 0);        //alpha between 0 and pi/2
				wma_dec_j3 = fixmul32(wma_dec_j2, wma_dec_alpha);
				wma_dec_j3 = wma_dec_j3 << 16;
				window[wma_dec_j] = fsincos(wma_dec_j3, 0);        //alpha between 0 and pi/2
			}
			s->windows[wma_dec_j] = window;
		}
    ...
    if (s->use_noise_coding)
    {
        ...
        {
            s->noise_mult = 0x51f;
            s->noise_table = (int32_t *)noisetable_exp1;
        }
        else
        {
            s->noise_mult = 0xa3d;
            /* LSP values are simply 2x the EXP values */
// Magicoe            for (i=0;i<NOISE_TAB_SIZE;++i)
//                noisetable_exp[i] = noisetable_exp[i]<< 1;
            s->noise_table = (int32_t *)noisetable_exp2;
        }
...
// Magicoe    runtabarray[0] = runtab0; runtabarray[1] = runtab1;
// Magicoe    levtabarray[0] = levtab0; levtabarray[1] = levtab1;

	runtabarray[0] = (uint16_t *)runtab0;
	runtabarray[1] = (uint16_t *)runtab1;
	levtabarray[0] = (uint16_t *)levtab0;
	levtabarray[1] = (uint16_t *)levtab1;
