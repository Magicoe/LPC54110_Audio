...
extern volatile char* ogg_ext_buffer;
extern volatile int   ogg_ext_total;
...
{
	int *temp;
//	rt_kprintf(" alloca %d %d\r\n", size, ogg_ext_total);
	
	if( ogg_ext_total < OGG_EXT_BUFFERSIZE) {
		temp = (int * )&ogg_ext_buffer[ogg_ext_total+128];
		ogg_ext_total += size ;
	}
	else {
	  ogg_ext_total = 0;
	  temp = (int * )&ogg_ext_buffer[ogg_ext_total];
	}
