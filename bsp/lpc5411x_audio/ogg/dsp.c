...
vorbis_info *vb_dsp_vi;
codec_setup_info *vb_dsp_ci;
...
//  codec_setup_info     *ci=(codec_setup_info *)vi->codec_setup;
  int                   mode,i;
	vb_dsp_vi=vd->vi;
	vb_dsp_ci=(codec_setup_info *)vb_dsp_vi->codec_setup;
  ...
  /* read our mode and pre/post windowsize */
  mode=oggpack_read(&vd->opb,ilog(vb_dsp_ci->modes));
  if(mode==-1 || mode>=vb_dsp_ci->modes) return OV_EBADPACKET;

  /* shift information we still need from last window */
  vd->lW=vd->W;
  vd->W=vb_dsp_ci->mode_param[mode].blockflag;
	
  for(i=0;i<vb_dsp_vi->channels;i++) {
    mdct_shift_right(vb_dsp_ci->blocksizes[vd->lW],vd->work[i],vd->mdctright[i]);	
  }
  ...
  if(decodep){
    mapping_inverse(vd,vb_dsp_ci->map_param+vb_dsp_ci->mode_param[mode].mapping);
    if(vd->out_begin==-1){
      vd->out_begin=0;
      vd->out_end=0;
    }else{
      vd->out_begin=0;
      vd->out_end=vb_dsp_ci->blocksizes[vd->lW]/4+vb_dsp_ci->blocksizes[vd->W]/4;
    }
  }
  ...
  if(vd->sample_count==-1){
    ...
    vd->sample_count+=
      vb_dsp_ci->blocksizes[vd->lW]/4+vb_dsp_ci->blocksizes[vd->W]/4;
  }
