...
// Magicoe static WMADecodeContext wmadec;
...
#define STREAM_BUFFER_SIZE		1024 //4096 //
...
rt_size_t stream_buffer_read(struct stream_buffer* stream, rt_uint8_t* ptr, rt_size_t len)
{
	...
	{
		rest_bytes = stream->position - stream->length;
...
struct stream_buffer* stream_buffer_create(int fd)
{
	...
	if (buffer != RT_NULL)
	{
		...
		if(buffer->buffer == NULL) {
			rt_kprintf("malloc buffer->buffer failed\n");
			while(1);
		}
		rt_kprintf("---------------------\r\n");
		rt_kprintf("buffer->buffer malloc size %d addr 0x%x\r\n", STREAM_BUFFER_SIZE, buffer->buffer);			
		rt_kprintf("---------------------\r\n");
	}
	return buffer;
}
...
void stream_buffer_close(struct stream_buffer* stream)
{
	...
	rt_kprintf("---------------------\r\n");
	rt_kprintf("release stream buffer\r\n");
	rt_kprintf("---------------------\r\n");
	close(stream->fd);
}
...
{
	if((guid1->v1 != guid2->v1) || (guid1->v2 != guid2->v2) || (guid1->v3 != guid2->v3) || (rt_memcmp(guid1->v4, guid2->v4, 8)))
	{
		return 0;
	}
	return 1;
...
rt_bool_t get_asf_metadata(struct stream_buffer* stream, struct id3_tag* id3)
{
	...
	if(wfx == NULL) {
		rt_kprintf("malloc wfx failed\n");
		while(1);
	}
	rt_kprintf("wfx malloc size %d addr 0x%x\r\n", sizeof(asf_waveformatex_t), wfx);		
...
extern volatile fixed32 *coefsarray;
extern volatile fixed32 *frame_out_buf;
extern volatile fixed32 *stat0;
extern volatile fixed32 *stat1;
extern volatile fixed32 *stat2;
extern volatile fixed32 *stat3;
extern volatile fixed32 *stat4;
extern volatile uint16_t *runtab0;
extern volatile uint16_t *runtab1;
extern volatile uint16_t *levtab0;
extern volatile uint16_t *levtab1; 
extern VLC_TYPE (*vlcbuf1)[2];//[VLCBUF1SIZE][2];
extern VLC_TYPE (*vlcbuf2)[2];//[VLCBUF2SIZE][2];
extern VLC_TYPE (*vlcbuf3)[2];//[VLCBUF3SIZE][2];
extern VLC_TYPE (*vlcbuf4)[2];//[VLCBUF4SIZE][2];
extern void codec_removed(void);
...
void wma(const char* filename)
{
	...
	wmadec = rt_malloc(sizeof(WMADecodeContext));
	if(wmadec == NULL) {
		rt_kprintf("malloc wmadec failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	coefsarray = rt_malloc(MAX_CHANNELS*BLOCK_MAX_SIZE*4);
	if(coefsarray == NULL) {
		rt_kprintf("malloc coefsarray failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	frame_out_buf = rt_malloc(MAX_CHANNELS*BLOCK_MAX_SIZE*2*4);
	if(frame_out_buf == NULL) {
		rt_kprintf("malloc frame_out_buf failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	runtab0 = rt_malloc(1336*2);
	if(runtab0 == NULL) {
		rt_kprintf("malloc runtab0 failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	runtab1 = rt_malloc(1336*2);
	if(runtab1 == NULL) {
		rt_kprintf("malloc runtab1 failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	levtab0 = rt_malloc(1336*2);
	if(levtab0 == NULL) {
		rt_kprintf("malloc levtab0 failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	levtab1 = rt_malloc(1336);
	if(levtab1 == NULL) {
		rt_kprintf("malloc levtab1 failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	vlcbuf1 = rt_malloc(4598*4);
	if(vlcbuf1 == NULL) {
		rt_kprintf("malloc vlcbuf1 failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	vlcbuf2 = rt_malloc(3574*4);
	if(vlcbuf2 == NULL) {
		rt_kprintf("malloc vlcbuf2 failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	vlcbuf3 = rt_malloc(360*4);
	if(vlcbuf3 == NULL) {
		rt_kprintf("malloc vlcbuf3 failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
	...
	vlcbuf4 = rt_malloc(540*4);
	if(vlcbuf4 == NULL) {
		rt_kprintf("malloc vlcbuf4 failed\n");
		rt_kprintf("---------------------\r\n");
		list_mem();
		rt_kprintf("---------------------\r\n");
		while(1);
...
// Magicoe   if (wma_decode_init(&wmadec, wfx) < 0) {
...
//    }
	...
	rt_kprintf("sample_rate %d\r\n", wmadec->sample_rate );
	rt_kprintf("nb_channels %d\r\n", wmadec->nb_channels );
	rt_kprintf("version %d\r\n", wmadec->version );
	rt_kprintf("use_noise_coding %d\r\n", wmadec->use_noise_coding );
...
// Magicoe            wma_decode_superframe_init(&wmadec, audiobuf, audiobufsize);
			wma_decode_superframe_init(wmadec, audiobuf, audiobufsize);
// Magicoe           for (i=0; i < wmadec.nb_frames; i++)
			for (i=0; i < wmadec->nb_frames; i++) {
// Magicoe                wmares = wma_decode_superframe_frame(&wmadec,
//                                                     (rt_uint32_t *)&play_temp_buffer[0], //decoded,
