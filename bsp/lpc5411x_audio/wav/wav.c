...
void wav(char* filename)
{
    ...
    if (fd >= 0) {
        ...
        rt_device_open(device, RT_DEVICE_OFLAG_WRONLY);
				...
				temp = Chip_Clock_GetMainClockRate();
				rt_kprintf("MainClk %d  ", temp);
				temp = Chip_Clock_GetSystemClockRate();
				rt_kprintf("SysClk %d  \r\n", temp);
				rt_sem_take(&wav_play_sem, 100);
        do
        {
						if(wav_read_index == 0) {
							len = read(fd, (char*)wav_buf0, 8192);
						}
						if(wav_read_index == 1) {
							len = read(fd, (char*)wav_buf1, 8192);
						}
					  wav_buf_size = 8192;
						if(wav_play_flag == 0) wav_play_flag = 1;
//						rt_kprintf("wav read index %d size %d\r\n", wav_read_index, wav_buf_size);
						rt_sem_take(&wav_play_sem, RT_WAITING_FOREVER);
        }
        while (len != 0);
				rt_kprintf("closed wav paly\r\n");		
