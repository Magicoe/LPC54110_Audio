...
{
	uint32_t idx = PININT_SRC_BITCFG_START + (sliceNum * 3);

	/* Configure bit slice configuration */
	pPININT->PMCFG = (pPININT->PMCFG & ~(PININT_SRC_BITCFG_MASK << idx)) | (slice_cfg << idx);

	/* If end point is true, enable the bits */
	if (end_point && sliceNum != PININTBITSLICE7) {
